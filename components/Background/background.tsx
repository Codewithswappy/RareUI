...
export default function InteractiveBackground({ className, children }: InteractiveBackgroundProps) {
  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      constructor() {
        ...
        const r = Math.random();
        if (r > 0.8)
          this.color = 'rgba(100, 200, 255, 0.8)'; // Cyan
        else if (r > 0.9)
          this.color = 'rgba(200, 100, 255, 0.8)'; // Purple
  ...
  return (
    <div className={cn('relative h-full w-full overflow-hidden bg-[#050505]', className)}>
      {/* Premium Gradient Background Layers */}
      <div className="bg-radial-gradient(ellipse_at_top,_var(--tw-gradient-stops)) absolute inset-0 z-0 from-slate-900/50 via-gray-900 to-black" />
      ...
      <motion.div
        ...
        className="pointer-events-none absolute top-[-30%] left-[20%] h-[60%] w-[60%] rounded-full bg-indigo-500/10 blur-[150px]"
      />
      ...
      <motion.div
        ...
        className="pointer-events-none absolute right-[-10%] bottom-[-20%] h-[50%] w-[50%] rounded-full bg-cyan-500/5 blur-[150px]"
      />

      {/* Canvas for Particles */}
      <canvas
        ref={canvasRef}
        className="pointer-events-auto absolute inset-0 z-10 h-full w-full cursor-none" // hidden cursor for immersion? or normal
