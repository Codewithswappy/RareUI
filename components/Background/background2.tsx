...
export default function InteractiveBackground2({
    ...
    class Particle {
      ...
      draw() {
        ...
        const drawColor = this.color
          .replace('0.8', alpha.toString())
          .replace('0.5', alpha.toString());
  ...
  return (
    <div className={cn('relative h-full w-full overflow-hidden bg-[#0a0a0a]', className)}>
      {/* Vignette */}
      <div className="pointer-events-none absolute inset-0 z-10 bg-[radial-gradient(circle_at_center,transparent_0%,rgba(0,0,0,0.8)_90%)]" />

      <canvas ref={canvasRef} className="pointer-events-auto absolute inset-0 h-full w-full" />

      <div className="pointer-events-none relative z-20 h-full w-full">{children}</div>
    </div>
  );
}
