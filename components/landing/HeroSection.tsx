    ...
    <div className="min-h-screen font-sans transition-colors duration-300 bg-background text-foreground selection:bg-primary/20">
      ...
      <nav className="max-w-7xl mx-auto px-6 py-6 h-16 flex items-center justify-between relative z-50">
        <div className="flex items-center z-50">
           ...
           <div className="relative w-[50px] h-[50px] md:w-[72px] md:h-[72px] rounded-full overflow-hidden flex justify-center">
             <Image 
               src="/RareUI_Logo.svg"
               alt="RareUI Logo" 
               width={72} 
        ...
        <div className="flex items-center gap-2 md:gap-4 z-50">
           ...
           <button
             className="md:hidden p-2 flex flex-col justify-center items-center gap-1.5 w-10 h-10 z-50"
             onClick={() => setIsMenuOpen(!isMenuOpen)}
           >
             <motion.span 
               animate={isMenuOpen ? { rotate: 45, y: 8 } : { rotate: 0, y: 0 }}
               className={`w-6 h-0.5 block rounded-full transition-colors ${isDark ? 'bg-white' : 'bg-black'}`}
             />
             <motion.span 
