...
// Remove background styles from themes to prevent hydration mismatch with custom transparent background
const cleanTheme = (theme: any) => {
  const newTheme = { ...theme };
  // Remove root level background properties if any (SyntaxHighlighter applies these to the pre)
  // But usually SyntaxHighlighter applies theme keys to elements. 
  // We want to ensure the 'hljs' or 'code[class*="language-"]' or 'pre[class*="language-"]' 
  // doesn't have background set if we want to override it cleanly.
  // Actually, react-syntax-highlighter themes are objects where keys are selectors.
  // We need to find the root selectors and remove background.
               ...
               <div className="flex items-center gap-2">
                   ...
                   <div className="flex items-center p-1 bg-muted/50 rounded-lg border border-border/50">
                      <button 
                         ...
                         className={cn("p-1.5 rounded  transition-colors", previewWidth === "100%" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground")}
                         title="Desktop"
                      >
                      ...
                      <button 
                         ...
                         className={cn("p-1.5 rounded transition-colors", previewWidth === "768px" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground")}
                         title="Tablet"
                      >
                      ...
                      <button 
                         ...
                         className={cn("p-1.5 rounded transition-colors", previewWidth === "375px" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground")}
                         title="Mobile"
                      >
                   ...
                   <button 
                      ...
                      className={cn("p-2 text-muted-foreground hover:text-foreground bg-muted/50 hover:bg-muted rounded-lg transition-colors", isLoading && "animate-spin")}
                ...
                <div className="flex-1 overflow-auto p-0 md:p-8 flex relative z-10">
                   {activeTab === "preview" ? (
                       <motion.div 
                          ...
                          className={cn(
                             "bg-background md:rounded-xl md:border md:border-border/50 md:shadow-xl overflow-hidden relative m-auto",
                             previewWidth === "100%" ? "w-full" : ""
                          )}
                       >
                           <div key={renderKey} className={cn("min-h-[350px] w-full flex items-center justify-center", previewClassName)}>
                               {children}
                           </div>
                       </motion.div>
                   ) : (
                       <div className="w-full h-full relative group">
                           <div className="absolute right-4 top-4 z-20 flex gap-2">
                                <div className="flex items-center gap-1">
                                    <button 
                                        ...
                                        className={cn("p-1.5 rounded transition-all duration-200", language === "tsx" ? "scale-110 opacity-100" : "opacity-40 hover:opacity-100 grayscale hover:grayscale-0")}
                                        title="TypeScript"
                                    >
                                        {ICONS.ts}
                                    </button>
                                    <button 
                                        ...
                                        className={cn("p-1.5 rounded transition-all duration-200", language === "jsx" ? "scale-110 opacity-100" : "opacity-40 hover:opacity-100 grayscale hover:grayscale-0")}
                                        title="JavaScript"
                                    >
                                        {ICONS.js}
                                    </button>
                           ...
                           <div className={cn("rounded-xl overflow-hidden border border-border/50 bg-zinc-50 dark:bg-[#09090b] relative", !expanded && "max-h-[400px]")}>
                                {mounted ? (
                                    <SyntaxHighlighter
                                        language={language}
                                        style={theme === "light" ? vsClean : materialDarkClean}
                                        customStyle={{ margin: 0, padding: '1.5rem', paddingBottom: '3rem', background: 'transparent', fontSize: '0.875rem' }}
                                    >
                                        {displayCode || ""}
                                    </SyntaxHighlighter>
                                ) : (
                                ...
                                {!expanded && (
                                    <div className={cn(
                                        ...
                                        theme === "light" ? "from-zinc-50 to-transparent" : "from-[#09090b] to-transparent"
                                    )}>
                                        <button 
                ...
                <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-[#09090b] overflow-hidden shadow-sm">
                    ...
                    <div className="flex items-center bg-transparent">
                        {mounted ? (
                            ...
                            <pre className="p-4 text-sm opacity-0">{getPackageManagerCommand()}</pre>
                        )}
                    </div>
                </div>
            </>
