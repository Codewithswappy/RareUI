...
export async function add(components: string[], options: AddOptions) {
  ...
  if (!(await fs.pathExists(componentsJsonPath))) {
    p.log.error(
      'components.json not found. Please run ' + chalk.cyan('npx rareui init') + ' first.'
    );
    process.exit(1);
  }
  ...
  if (components.length === 0) {
    ...
    console.log(
      chalk.dim('  • ') +
        chalk.cyan('npx rareui add [component]') +
        chalk.dim(' - Add a specific component')
    );
    console.log(
      chalk.dim('  • ') + chalk.cyan('npx rareui add .') + chalk.dim(' - Add all components')
    );
    console.log();
    process.exit(0);
  }
  ...
  for (const inputName of components) {
    ...
    try {
      ...
      p.log.error(
        `Error adding component: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
